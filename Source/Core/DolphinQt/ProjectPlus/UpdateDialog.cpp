...
void UpdateDialog::accept()
{
    ...
    for (const QJsonValue& value : jsonArray)
    {
        ...
        #ifdef _WIN32
        if (filenameBlob.contains(QStringLiteral("Revy.Windows.Update")) || 
            filenameBlob.contains(QStringLiteral("Revy.Windows")) || 
            filenameBlob.contains(QStringLiteral("Revy.win64")))
        {
        ...
        #endif
        #ifdef __APPLE__
        if (filenameBlob.contains(QStringLiteral("Revy.macOS.Update")) || 
            filenameBlob.contains(QStringLiteral("Revy.macOS")))
        {
            filenameToDownload = filenameBlob;
            urlToDownload = downloadUrl;
            break;
