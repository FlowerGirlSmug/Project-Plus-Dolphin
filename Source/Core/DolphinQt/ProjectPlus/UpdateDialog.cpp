...
void UpdateDialog::accept()
{
    ...
    for (const QJsonValue& value : jsonArray)
    {
        ...
        #ifdef _WIN32
        if (filenameBlob.contains(QStringLiteral("CO.Windows.Update")) || 
            filenameBlob.contains(QStringLiteral("CO.Windows")) || 
            filenameBlob.contains(QStringLiteral("CO.win64")))
        {
        ...
        #endif
        #ifdef __APPLE__
        if (filenameBlob.contains(QStringLiteral("CO.macOS.Update")) || 
            filenameBlob.contains(QStringLiteral("CO.macOS")))
        {
            filenameToDownload = filenameBlob;
            urlToDownload = downloadUrl;
            break;
