...
void MainWindow::ShowUpdateDialog()
{
  ...
  QJsonObject latestCO = GetLatestCORelease(releases);

  if (latestCO.isEmpty())
  {
    QMessageBox::information(this, tr("Info"), tr("No Colorado releases found."));
    return;
  }

  QString currentVersion = QString::fromStdString(SCM_DESC_STR);
  QString latestVersion = latestCO.value(QStringLiteral("tag_name")).toString();

  if (currentVersion != latestVersion)
  {
    bool forced = false;
    UserInterface::Dialog::UpdateDialog updater(this, latestCO, forced);
    updater.exec();
  }
  else
  {
...
void MainWindow::CheckForUpdatesAuto()
{
  ...
  QJsonObject latestCO = GetLatestCORelease(jsonDoc.array());
  if (latestCO.isEmpty())
    return;
